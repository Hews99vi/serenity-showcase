...
const FeaturedFilms = () => {
  return <section className="py-16 md:py-24 bg-charcoal">
      <div className="section-container">
        {/* Section Header */}
        <div className="flex items-center justify-between mb-10">
          <h2 className="text-2xl md:text-3xl font-serif text-cream tracking-wider uppercase">OUR WORK</h2>
          <Link to="/portfolio" className="text-cream/60 text-xs tracking-[0.2em] uppercase hover:text-cream transition-colors duration-300 link-underline">
            view all
          </Link>
        </div>
        ...
        <div className="grid grid-cols-3 gap-3 md:gap-4">
          {featuredFilms.slice(0, 6).map(film => <div key={film.id} className="group">
              {/* Poster Card - Instagram Reels Aspect Ratio */}
              <div className="relative aspect-[9/16] overflow-hidden bg-charcoal-light rounded-sm">
                {film.youtubeId ? <iframe src={`https://www.youtube.com/embed/${film.youtubeId}?autoplay=1&mute=1&loop=1&playlist=${film.youtubeId}&controls=0&showinfo=0&rel=0&modestbranding=1&playsinline=1`} className="absolute inset-0 w-full h-full object-cover" allow="autoplay; encrypted-media" allowFullScreen title={film.coupleName} style={{
              border: 'none',
              pointerEvents: 'none'
            }} /> : <img src={film.thumbnail} alt={film.coupleName} className="absolute inset-0 w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" loading="lazy" />}
                ...
                {!film.youtubeId && <Link to="/portfolio" className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
